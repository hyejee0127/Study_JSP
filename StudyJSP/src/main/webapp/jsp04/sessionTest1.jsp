...
<body>
    ...
    - 세션 유지 시간과 관계없이 session 객체의 invalidate() 메서드를 호출하면 세션 초기화 됨
    - 또한, 웹브라우저 종료하면 세션이 초기화됨
      (웹브라우저가 다르면 세션도 달라진다!)
	--%>
	...
	<%session.invalidate(); %>
	<!-- 세션을 초기화하게 되면 session 객체 자체에 아무것도 저장되지 않는 상태와 동일하게 됨 -->
	<!-- 따라서, session 객체에 접근할 경우 오류 발생하게 된다! -->
<%-- 	<h3>새 세션 여부 : <%=session.isNew() %></h3> <!-- 이 코드가 실행될 경우 오류 발생! --> --%>
